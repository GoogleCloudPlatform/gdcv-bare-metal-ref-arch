...
print_and_execute "gcloud compute firewall-rules delete --quiet ${firewall_rule_name}"
...
for cluster_name in $(get_cluster_names); do
    ...
    print_and_execute "gcloud compute forwarding-rules delete --quiet ${forwarding_rule_name} --global"
    ...
    print_and_execute "gcloud compute target-http-proxies delete --quiet ${http_proxy_name}"

    tcp_proxy_name=${cluster_name}-asm-tcp-proxy
    bold_no_wait "Deleting TCP proxy '${tcp_proxy_name}'"
    print_and_execute "gcloud compute target-tcp-proxies delete --quiet ${tcp_proxy_name}"

    url_map_name=${cluster_name}-asm-url-map
    bold_no_wait "Deleting URL map '${url_map_name}'"
    print_and_execute "gcloud compute url-maps delete --quiet  ${url_map_name}"

    backend_80_name=${cluster_name}-asm-80-lb
    bold_no_wait "Deleting backend '${backend_80_name}'"
    print_and_execute "gcloud compute backend-services delete --quiet ${backend_80_name} --global"
    ...
    print_and_execute "gcloud compute network-endpoint-groups delete --quiet ${neg_443_name} --zone=${ZONE}"
