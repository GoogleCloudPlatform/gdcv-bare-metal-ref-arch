...
print_and_execute "gcloud compute health-checks create tcp ${health_check_name} --use-serving-port"
...
for cluster_name in $(get_cluster_names); do
    ...
    address=$(gcloud compute addresses describe ${address_name} --global --format "value(address)")
...
EOF
    ...
    print_and_execute "gcloud compute network-endpoint-groups create ${neg_80_name} --network-endpoint-type=GCE_VM_IP_PORT --zone=${ZONE} --network=default"

    neg_443_name=${cluster_name}-asm-443-neg
    bold_no_wait "Creating network endpoint group '${neg_443_name}'"
    print_and_execute "gcloud compute network-endpoint-groups create ${neg_443_name} --network-endpoint-type=GCE_VM_IP_PORT --zone=${ZONE} --network=default"
    ...
    print_and_execute "gcloud compute firewall-rules create ${firewall_rule_name} --allow tcp:${http_node_port},tcp:${https_node_port} --source-ranges 130.211.0.0/22,35.191.0.0/16 --target-tags ${firewall_rule_tag}"
    ...
    for worker in $(seq 1 $(get_number_of_worker_nodes)); do
        ...
        print_and_execute "gcloud compute network-endpoint-groups update ${neg_80_name} --zone=${ZONE} --add-endpoint='instance=${hostname},port=${http_node_port}'"

        bold_no_wait "Adding ${hostname} in ${ZONE} to ${neg_443_name}"
        print_and_execute "gcloud compute network-endpoint-groups update ${neg_443_name} --zone=${ZONE} --add-endpoint='instance=${hostname},port=${https_node_port}'"
    ...
    print_and_execute "gcloud compute backend-services create ${backend_80_name} --global --health-checks=${health_check_name} --protocol=HTTP"

    bold_no_wait "Adding '${neg_80_name}' to '${backend_80_name}'"
    print_and_execute "gcloud compute backend-services add-backend ${backend_80_name} --balancing-mode=RATE --global --max-rate=1000 --network-endpoint-group=${neg_80_name} --network-endpoint-group-zone=${ZONE}"
    
    url_map_name=${cluster_name}-asm-url-map
    bold_no_wait "Creating URL map '${url_map_name}'"
    print_and_execute "gcloud compute url-maps create ${url_map_name} --default-service=${backend_80_name}"

    http_proxy_name=${cluster_name}-asm-http-proxy
    bold_no_wait "Creating HTTP proxy '${http_proxy_name}'"
    print_and_execute "gcloud compute target-http-proxies create ${http_proxy_name} --url-map=${url_map_name}"

    forwarding_rule_name=${cluster_name}-asm-80-forwarding-rule
    bold_no_wait "Creating forwarding rule '${forwarding_rule_name}'"
    print_and_execute "gcloud compute forwarding-rules create ${forwarding_rule_name} --address=${address_name} --global --ports=80 --target-http-proxy=${http_proxy_name}"

    backend_443_name=${cluster_name}-asm-443-lb
    bold_no_wait "Creating backend '${backend_443_name}'"
    print_and_execute "gcloud compute backend-services create ${backend_443_name} --global --health-checks=${health_check_name} --protocol=TCP"

    bold_no_wait "Adding '${neg_443_name}' to '${backend_443_name}'"
    print_and_execute "gcloud compute backend-services add-backend ${backend_443_name} --balancing-mode=CONNECTION --global --max-connections=1000 --network-endpoint-group=${neg_443_name} --network-endpoint-group-zone=${ZONE}"

    tcp_proxy_name=${cluster_name}-asm-tcp-proxy
    bold_no_wait "Creating TCP proxy '${tcp_proxy_name}'"
    print_and_execute "gcloud compute target-tcp-proxies create ${tcp_proxy_name} --backend-service=${backend_443_name}"
