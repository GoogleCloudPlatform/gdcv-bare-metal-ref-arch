...
grep -q "export ASM_VERSION_MINOR=[${VALID_CHARACTERS}]\+$" ${ENVIRONMENT_FILE} || echo -e "export ASM_VERSION_MINOR=${ASM_VERSION_MINOR:-11}" >> ${ENVIRONMENT_FILE}
grep -q "export ASM_VERSION_POINT=[${VALID_CHARACTERS}]\+$" ${ENVIRONMENT_FILE} || echo -e "export ASM_VERSION_POINT=${ASM_VERSION_POINT:-2}" >> ${ENVIRONMENT_FILE}
grep -q "export ASM_VERSION_REV=[${VALID_CHARACTERS}]\+$" ${ENVIRONMENT_FILE} || echo -e "export ASM_VERSION_REV=${ASM_VERSION_REV:-17}" >> ${ENVIRONMENT_FILE}
grep -q "export ASM_VERSION_CONFIG=[${VALID_CHARACTERS}]\+$" ${ENVIRONMENT_FILE} || echo -e "export ASM_VERSION_CONFIG=${ASM_VERSION_CONFIG:-2}" >> ${ENVIRONMENT_FILE}
grep -q "export BILLING_ACCOUNT_ID=" ${ENVIRONMENT_FILE} || echo -e "export BILLING_ACCOUNT_ID=${BILLING_ACCOUNT_ID}" >> ${ENVIRONMENT_FILE}
grep -q "export BMCTL_VERSION=[${VALID_CHARACTERS}]\+$" ${ENVIRONMENT_FILE} || echo -e "export BMCTL_VERSION=${BMCTL_VERSION:-1.9.0}" >> ${ENVIRONMENT_FILE}
...
grep -q "export CLOUD_SDK_VERSION=[${VALID_CHARACTERS}]\+$" ${ENVIRONMENT_FILE} || echo -e "export CLOUD_SDK_VERSION=${CLOUD_SDK_VERSION:-360.0.0}" >> ${ENVIRONMENT_FILE}
