...
resource "google_compute_firewall" "gdcv_allow_cp_healthcheck" {
  name          = "${local.gdcv_network_name}-allow-cp-healthcheck"
  network       = data.google_compute_network.gdcv_prod.id
  project       = local.project_id_gdcv_on_gce_prod
  source_ranges = data.google_netblock_ip_ranges.health_checkers.cidr_blocks
  target_tags   = [var.gdcv_node_tag_cp]
...
resource "google_compute_health_check" "gdcv_cp_healthcheck" {
  name    = "${local.gdcv_network_name}-cp-healthcheck"
  project = local.project_id_gdcv_on_gce_prod
