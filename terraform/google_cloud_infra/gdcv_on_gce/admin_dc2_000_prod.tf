...
locals {
  admin_dc2_000_prod_startup_script = templatefile(
...
resource "google_service_account" "admin_dc2_000_prod_sa" {
  account_id   = var.admin_dc2_000_prod_cluster_name
  display_name = "${var.admin_dc2_000_prod_cluster_name} service account"
  project      = data.google_project.gdcv_on_gce_prod.project_id
}

resource "google_project_iam_member" "admin_dc2_000_prod_sa_logging_logwriter" {
  member  = google_service_account.admin_dc2_000_prod_sa.member
  project = data.google_project.gdcv_on_gce_prod.project_id
  role    = "roles/logging.logWriter"
}

resource "google_secret_manager_secret_iam_member" "gdcv_ssh_public_key_admin_dc2_000_prod_sa" {
  member    = google_service_account.admin_dc2_000_prod_sa.member
...
resource "google_compute_instance" "admin_dc2_000_prod_cp" {
  count = var.admin_dc2_000_prod_cp_nodes

  machine_type            = "n2-standard-8"
  metadata_startup_script = local.admin_dc2_000_prod_startup_script
  ...
  service_account {
    email  = google_service_account.admin_dc2_000_prod_sa.email
