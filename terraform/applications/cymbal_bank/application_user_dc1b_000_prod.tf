...
locals {
  fwi_identity_provider_user_dc1b_000_prod = "https://gkehub.googleapis.com/projects/${local.project_id_fleet_prod}/locations/global/memberships/${var.user_dc1b_000_prod_cluster_name}"
  kubeconfig_user_dc1b_000_prod            = "${local.kubeconfig_dir}/${var.user_dc1b_000_prod_cluster_name}"
}
...
provider "kubernetes" {
  ...
  config_path = local.kubeconfig_user_dc1b_000_prod
}
...
###############################################################################
# resource "kubernetes_config_map" "accounts_db_config_user_dc1b_000_prod" {
...
#     namespace = kubernetes_namespace.cymbal_bank_user_dc1b_000_prod.id
#   }
# }

resource "kubernetes_secret" "artifact_registry_user_dc1b_000_prod" {
  provider = kubernetes.user_dc1b_000_prod
...
resource "kubernetes_config_map" "cloud_sql_admin_user_dc1b_000_prod" {
  provider = kubernetes.user_dc1b_000_prod
...
resource "kubernetes_config_map" "demo_data_config_user_dc1b_000_prod" {
  provider = kubernetes.user_dc1b_000_prod
...
resource "kubernetes_config_map" "environment_config_user_dc1b_000_prod" {
  provider = kubernetes.user_dc1b_000_prod
...
resource "kubernetes_config_map" "fwi_backend_adc_user_dc1b_000_prod" {
  provider = kubernetes.user_dc1b_000_prod
  ...
  data = {
    config = templatefile(
      "${path.module}/templates/fwi_adc.jsontpl",
      {
        ...
        fwi_identity_provider        = local.fwi_identity_provider_user_dc1b_000_prod
...
resource "kubernetes_config_map" "fwi_frontend_adc_user_dc1b_000_prod" {
  provider = kubernetes.user_dc1b_000_prod
...
# resource "kubernetes_config_map" "ledger_db_config_user_dc1b_000_prod" {
#   provider = kubernetes.user_dc1b_000_prod
...
resource "kubernetes_config_map" "service_api_config_user_dc1b_000_prod" {
  provider = kubernetes.user_dc1b_000_prod
...
###############################################################################
resource "kubernetes_namespace" "cymbal_bank_user_dc1b_000_prod" {
  provider = kubernetes.user_dc1b_000_prod
...
# KUBERNETES SECRETES
###############################################################################
resource "kubernetes_secret" "jwt_user_dc1b_000_prod" {
  provider = kubernetes.user_dc1b_000_prod
...
###############################################################################
resource "kubernetes_service_account" "cymbal_bank_backend_user_dc1b_000_prod" {
  provider = kubernetes.user_dc1b_000_prod

  image_pull_secret {
    name = kubernetes_secret.artifact_registry_user_dc1b_000_prod.metadata.0.name
  }
  metadata {
    annotations = {
...
resource "kubernetes_service_account" "cymbal_bank_frontend_user_dc1b_000_prod" {
  provider = kubernetes.user_dc1b_000_prod
...
# DEPLOYMENT
###############################################################################
resource "null_resource" "deploy_microservices_user_dc1b_000_prod" {
  depends_on = [
    google_sql_database.accounts,
    google_sql_database.ledger,
    kubernetes_service_account.cymbal_bank_backend_user_dc1b_000_prod,
    kubernetes_service_account.cymbal_bank_frontend_user_dc1b_000_prod,
  ...
  triggers = {
    ...
    kubeconfig                 = local.kubeconfig_user_dc1b_000_prod
  }
...
output "ingress_url_user_dc1b_000_prod" {
  value = "http://${var.user_dc1b_000_prod_ingress_vip}/"
}
